...
import {
    CustomerAddressPostalCodeMother
} from "@test/mother/core/Shop/Customer/domain/entity/CustomerAddressPostalCodeMother";
    ...
    it('should return 204 and update data', async () => {
        ...
        const customer = CustomerMother.create({
            ...
            address: CustomerAddressMother.create({
                address: CustomerAddressAddressMother.different(oldCustomer.address.address),
                city: CustomerAddressCityMother.different(oldCustomer.address.city),
                postalCode: CustomerAddressPostalCodeMother.different(oldCustomer.address.postalCode),
            }),
        ...
        const res = await app.request(`/customers/${customer.id.value}`, {method: 'PUT', body: JSON.stringify(body)})
